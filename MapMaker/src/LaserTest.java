...
public class LaserTest {
     ...
     *            normally 50000
     */
    public LaserTest(String host, int port) {
	robotcomm = new RobotCommunication(host, port);
        robotcomm.printAddress = false;
    }
	...
	LaserTest robot = new LaserTest("http://127.0.0.1", 50000);
	...
	for (int i = 0; i < 50; i++) {
	    ...
	    System.out.println("Object at " + echoes[56] + "m in " + angles[56] * 180.0 / Math.PI + " degrees");
            System.out.println("Echoes length: "+echoes.length+" Angles length: "+angles.length);
            for (int j=0; j<10; j++)
            {
                System.out.println((int)(angles[j*27]*180.0/Math.PI) +"deg : "+(int)echoes[j*27]+"m");
    ...
    /**
     * Gives the coordinates of a point picked up by the laser.
     * @param distance to the point in metres, one of the returned values from ler.getEchoes()
     * @param angle from the robot to the point in , one of the returned values from ler.getEchoes()
     * @param lr localization response object
     * @return 
     */
    private Position echoCoordinates(double distance, double laserAngle, LocalizationResponse lr){
        double[] rPos = getPosition(lr);
        double xr=rPos[0];
        double yr=rPos[1];
        double robotAngle = lr.getHeadingAngle();
        
        double x = xr + distance * Math.cos(laserAngle+robotAngle);
        double y = yr + distance * Math.sin(laserAngle+robotAngle);
